...
const Cerimonialista = () => {
    ...
    async function HandleConvidado(dados: DadosConvidado) {
        try {
            ...
            await api.patch(`/convidado/pesquisa?nome_completo${dados.nome_completo}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
    ...
    return (
        <main className="bg-amber-100 min-h-screen flex flex-col">
            <nav className="flex justify-between items-center border-b-3 border-amber-400 h-20 p-2">
